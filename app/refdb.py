...
def get_cache(filename):
    ...
    if os.path.exists(basedir + "/../cache/" + filename):
        with open(basedir + "/../cache/" + filename, 'r') as file:
            expiry = file.readline()
            expiry = datetime.strptime(expiry.strip(), '%Y/%m/%d %H:%M:%S')
            data = file.read()
    return expiry, data


def write_cache(filename, data, expiry=2):
    dt = datetime.now()
    expiry = dt + timedelta(minutes=expiry)
    with open(basedir + "/../cache/" + filename, "w") as f:
...
def get_refdb_pub():
    ...
    if expiry is not None and expiry > now:
        ...
        response = requests.get(BASE_URL + "/publickey")
        write_cache("publickey", response.text)
        key = response.text
    return key
